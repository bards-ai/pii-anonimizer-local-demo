...
async function classifyChunkText(text, offsetBase = 0) {
  let inputs = null;
  let output = null;
  ...
  try {
    ...
    for (let i = 0; i < inputIds.length; i++) {
      ...
      if (piece === "") {
        ...
        while (cursor < text.length && isAnyWhitespace(text[cursor])) cursor++;
        offsets.push(null);
        continue;
      }
    ...
    console.group(`%c[PII DEBUG] Token classification for: "${text.slice(0, 80)}..."`, "color: cyan; font-weight: bold");
    ...
    for (let i = 0; i < numTokens; i++) {
      ...
      const tokenText = text.slice(offset[0], offset[1]);
      ...
      results.push({
        ...
        score: bestScore,
        index: i,
        word: text.slice(offset[0], offset[1]),
        start: offset[0] + offsetBase,
        end: offset[1] + offsetBase,
      });
    }
    console.groupEnd();
...
self.onmessage = async (e) => {
  ...
  if (type === "load") {
    ...
    try {
      const sourceText = e.data.text;
      const chunks = Array.isArray(e.data.chunks) && e.data.chunks.length > 0
        ? e.data.chunks
        : [{ text: sourceText, start: 0, end: sourceText.length, wordCount: null }];
      const allResults = [];
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const chunkResults = await classifyChunkText(chunk.text, chunk.start);
        allResults.push(...chunkResults);
      }

      const merged = postprocessTokenResults(
